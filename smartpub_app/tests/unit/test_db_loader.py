...
class TestFindMostSimilarDocs:
    def test_returns_similarity_search_result(self, vdb):
        ...
        try:
            result = vdb.find_most_similar_docs(
                "COVID treatment", mock_index, num_of_chunks=2
            )
    ...
    def test_passes_k_to_similarity_search(self, vdb):
        ...
        try:
            ...
            mock_vs_cls.return_value.similarity_search.assert_called_once_with(
                "query", k=7
            )
        finally:
            db_loader.vectorstore_pc = original_vs
