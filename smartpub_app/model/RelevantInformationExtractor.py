...
def getTopSimilarDocs(question="Tell me about Schizophrenia?", num_docs=5, method=1):
    ...
    if method == 0:
        ...
        query_results = vectorstore_db.similarity_search(
            query=question, k=num_docs, namespace=namespace
        )
        # Format the output
        top_docs = []
    ...
    # STS using BERT
        ...
        for match in query_results["matches"]:
            ...
            document_tensor = torch.tensor(document_tokens).unsqueeze(
                0
            )  # Batch dimension
            # Get BERT embeddings for query and document relation text
            with torch.no_grad():
                ques_embedding = model(ques_tensor)[0][
                    :, 0, :
                ]  # First token (CLS token) pooled output
                document_embedding = model(document_tensor)[0][
                    :, 0, :
                ]  # First token (CLS token) pooled output
            doc_info = {
                "PMID": pmid,
            ...
            similarity_score = cosine_similarity(ques_embedding, document_embedding)[0][
                0
            ]
