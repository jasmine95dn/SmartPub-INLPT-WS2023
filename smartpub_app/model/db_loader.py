...
class PineconeVDB:
    ...
    def __init__(self, embedding_model_name: str, batch_size: int = 32):
        ...
        self.tokenizer = LlamaTokenizer.from_pretrained(
            "meta-llama/Llama-2-13b-chat-hf", token=hf_auth
        )
    ...
    def create_pinecone_index(
        self, index_name: str, metric: str = "cosine"
    ) -> pc.Index:
    ...
    def push_data_to_index(self, index_name: str, data: list[dict], namespace: str):
        ...
        for i in tqdm(
            ...
            desc=f"Pushing data to Index {index_name}:",
        ):
            ...
            for entry in batch:
                ...
                abstract_relations = readout_triplets(
                    entry["abstract_entity_relation"]["triplets"]
                )
                ...
                metadata.append(
                    {"relations": relation, "authors": authors, "date": date}
                )
    ...
    def find_most_similar_docs(
        self, query: str, index: pc.Index, num_of_chunks: int = 3
    ):
        vectorstore = vectorstore_pc(
            index=index, embedding=self.embedding_model, text_key="relations"
        )
        return vectorstore.similarity_search(query, k=num_of_chunks)
    ...
    def push_data_to_index(self, index_name: str, data: pd.DataFrame):
        ...
        for i in tqdm(
            ...
            for _, row in batch.iterrows():
                ...
                # get metadata to store in Pinecone
                metadata.append({"text": texts, "authors": authors, "date": date})
